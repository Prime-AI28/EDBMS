    ...
    def login():
        ...
        if pas == p:
            ...
            def Attendance_record():
                ...
                # Insert into tabel
                query = (
                    "CREATE TABLE `emp_info`.`emp_%s` ( `Date` DATE NOT NULL, `In_time` TIME NOT NULL, `Out_time` TIME NULL, PRIMARY KEY (`Date`))"
                    % (ID)
                )
            ...
            def update_screen():
                ...
                f_au = LabelFrame(
                    Updt, padx=10, pady=10, bg="#FFFFCC", relief=RIDGE, bd=6
                )
                ...
                Emp_ID_ed = Entry(
                    f_au,
                    width=30,
                    font=("Courier New", 16, "bold", "italic"),
                    bd=5,
                    justify="left",
                )
                ...
                save_btn = Button(
                    f_au, text="Save Record", command=update, padx=5, pady=5, bd=8
                )
                save_btn.grid(
                    row=10, column=1, columnspan=2, padx=10, pady=10, ipadx=98
                )
            ...
            def update():
                ...
                query = (
                    "UPDATE `emp_info`.`emp_data` SET  `Emp_Name`= '%s',  `Emp_Address`= '%s',  `Emp_Status`= '%s',  `Emp_DOB`= '%s',  `Emp_Salary`= '%s', `Emp_Passwd`= '%s',  `Emp_Remark`= '%s' WHERE (`Emp_ID` = '%s')"
                    % (
                        Emp_Name_ed.get(),
                        Emp_Address_ed.get(),
                        Emp_Status_ed.get(),
            ...
            add_btn = Button(
                f_ab, text="Add Record", command=submit, padx=5, pady=5, bd=8
            )
            add_btn.grid(row=1, column=1, columnspan=2, padx=10, pady=10, ipadx=88)
            # Create query Button
            query_btn = Button(
                f_ab, text="Show Records", command=query, padx=5, pady=5, bd=8
            )
            ...
            delete_btn = Button(
                f_ab, text="Delete Records", command=delete, padx=5, pady=5, bd=8
            )
            delete_btn.grid(row=4, column=1, columnspan=2, padx=10, pady=10, ipadx=80)
            # Create Update Button
            update_btn = Button(
                f_ab, text="Edit Record", command=update_screen, padx=5, pady=5, bd=8
            )
...
def Emp():
    ...
    def bill():
        ...
        catalog_category_dict = {
            "Sedan": "1 Sedan.txt",
            "Coupe": "2 Coupe.txt",
            "Sports Car": "3 Sports Car.txt",
            "Station Wagon": "4 Station Wagon.txt",
            "Hatchback": "5 Hatchback.txt",
            "Convertible": "6 Convertible.txt",
            "SUV": "7 SUV.txt",
        }
        ...
        def load_catalog():
            ...
            for file in catalog_file_list:
                ...
                while True:
                    ...
                    catalog_tabel.insert("", END, values=[name, price, category])
        ...
        style_button.configure(
            "TButton", font=("arial", 10, "bold"), background="darkgrey"
        )
        ...
        customer_name_entry = Entry(
            customer_frame, width=20, font="arial 15", bd=5, textvariable=customerName
        )
        ...
        combo_catalog = ttk.Combobox(
            catalog_category_frame,
            values=catalog_category,
            textvariable=catalogCategory,
        )
        combo_catalog.grid(row=0, column=1, padx=30)

        show_button = ttk.Button(
            catalog_category_frame, text="Show", width=10, command=show_button_operation
        )
        show_button.grid(row=0, column=2, padx=60)

        show_all_button = ttk.Button(
            catalog_category_frame, text="Show All", width=10, command=load_catalog
        )
        ...
        catalog_tabel = ttk.Treeview(
            catalog_tabel_frame,
            style="Treeview",
            columns=("name", "price", "category"),
            xscrollcommand=scrollbar_catalog_x.set,
        ...
        itemQuantity.set("")
        item_quantity = Entry(
            item_frame2, font="arial 12", textvariable=itemQuantity, width=10
        )
        ...
        clear_button = ttk.Button(
            item_frame3, text="Clear", width=8, command=clear_button_operation
        )
        ...
        bill_button = ttk.Button(
            order_frame, text="Bill", width=8, command=bill_button_operation
        )
        ...
        def check():
            ...
            def attmark():
                ...
                def outtime():
                    ...
                    d = conn.cursor()
                    query2 = (
                        "UPDATE `emp_info`.`emp_%s` SET `out_time` = '%s' WHERE (`date` = '%s')"
                        % (int(h.get()), z, a)
                    )
                ...
                frame1.pack()
                frame2.pack()
                b1 = Button(frame1, text="IN-TIME", command=intime, bd=8).grid(
                    row=0, column=0, columnspan=2, ipadx=50, pady=10, padx=50
                )
    ...
    # creating button
    bill_bttn = Button(
        frame1, text="Create A Bill", padx=28, pady=10, command=bill, bd=8
    )
    upd_bttn = Button(
        frame1, text="Update Address", padx=28, pady=10, command=update1, bd=8
    )
...
date = Label(
    f_m, textvariable=Date, font=("Castellar", 12, "italic"), fg="#546E7A", bg="#BBDEFB"
).grid(row=0, column=2)
